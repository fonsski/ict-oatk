...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
    <div>
        <label for="family" class="block text-gray-700 text-sm font-bold mb-2">Семейство</label>
        <input type="text" name="family" id="family" maxlength="100" list="os-families"
               class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('family') border-red-500 @enderror"
        ...
        <p class="text-xs text-gray-500 mt-1">Используется для группировки в списках</p>
        @error('family')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
    </div>

    <div>
        <label for="sort_order" class="block text-gray-700 text-sm font-bold mb-2">Порядок сортировки</label>
...
<div class="mb-4">
    ...
    <textarea name="description" id="description" rows="3" maxlength="1000"
              class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('description') border-red-500 @enderror">{{ old('description', $operatingSystem->description ?? '') }}</textarea>
...
<div class="mb-6">
    ...
    <p class="text-xs text-gray-500 mt-1">Неактивные ОС не предлагаются при выборе в карточке оборудования, но остаются у уже привязанной техники</p>
</div>
