...
<div class="max-w-7xl mx-auto px-6 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
            <h1 class="text-2xl font-bold text-slate-900 mb-6">Добавить расходник</h1>
            ...
            <form action="{{ route('consumables.store') }}" method="POST">
                ...
                <div class="grid grid-cols-3 gap-4 mb-4">
                    <div>
                        <label for="quantity" class="block text-slate-700 text-sm font-bold mb-2">Начальный остаток *</label>
                        <input type="number" name="quantity" id="quantity" min="0"
                               class="w-full rounded-lg border-slate-300 px-3 py-2 text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('quantity') border-red-500 @enderror"
                ...
                <div class="mb-4">
                    ...
                    <p class="text-sm text-slate-500 mt-1">Определяет, за каким отделом/мастером числится расходник</p>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" id="notes" rows="3" maxlength="1000"
                              class="w-full rounded-lg border-slate-300 px-3 py-2 text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('consumables.index') }}" class="text-slate-600 hover:text-slate-800 font-medium">
                        Отмена
                    </a>
                </div>
            </form>
