...
@section('title', 'Редактирование чертежа: ' . $drawing->title)

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 mb-2">Редактирование чертежа</h1>
            <p class="text-slate-600">Измените чертеж с помощью инструментов рисования</p>
        </div>
        <div class="flex items-center space-x-3">
            <a href="{{ route('drawing-canvas.show', $drawing) }}"
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('drawing-canvas.update', $drawing) }}" method="POST" id="drawing-form" class="p-0">
            @csrf
            @method('PUT')
                ...
                <div class="p-4 border-r border-slate-200 bg-slate-50">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea
                            ...
                            rows="3"
                            class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        >{{ old('description', $drawing->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select
                            ...
                            class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        >
                            <option value="general" {{ old('type', $drawing->type) == 'general' ? 'selected' : '' }}>Общий</option>
                            <option value="network" {{ old('type', $drawing->type) == 'network' ? 'selected' : '' }}>Сетевая диаграмма</option>
                            <option value="floorplan" {{ old('type', $drawing->type) == 'floorplan' ? 'selected' : '' }}>План помещения</option>
                            <option value="infrastructure" {{ old('type', $drawing->type) == 'infrastructure' ? 'selected' : '' }}>Инфраструктура</option>
                        </select>
                    </div>
    ...
    canvas.on('object:selected', function(options) {
        if (options.target) {
            if (options.target.stroke) {
                document.getElementById('stroke-color').value = options.target.stroke;
            }
            if (options.target.strokeWidth) {
                document.getElementById('stroke-width').value = options.target.strokeWidth;
            }
            if (options.target.fill && options.target.fill !== 'transparent') {
                document.getElementById('fill-color').value = options.target.fill;
            }
        }
    });
});
