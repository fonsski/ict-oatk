...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="bg-white shadow-md rounded-lg p-6">
            <h1 class="text-2xl font-bold text-gray-900 mb-2">Проведение закупки {{ $purchase->number }}</h1>
            ...
            <form action="{{ route('purchases.post', $purchase) }}" method="POST">
                ...
                <div class="mb-8">
                    <h2 class="text-lg font-semibold text-gray-900 mb-3">Расходники — пополнение остатка</h2>
                    <div class="overflow-x-auto border border-gray-200 rounded">
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Расходник</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Придёт</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Остаток сейчас</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Станет</th>
                                </tr>
                            </thead>
                ...
                <div class="mb-8">
                    <h2 class="text-lg font-semibold text-gray-900 mb-3">Оборудование — инвентарные номера</h2>
                    ...
                    <div class="border border-gray-200 rounded p-4 mb-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            ...
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">Единица {{ $i + 1 }} — инв. номер *</label>
                                <input type="text" name="inventory_numbers[{{ $item->id }}][]" required
                                       ...
                                       class="w-full rounded border-gray-300 px-3 py-2 @error('inventory_numbers.' . $item->id . '.' . $i) border-red-500 @enderror">
