...
- **Учётный номер** - выдается системой ICT Help (формат КодЗданияЭтаж-Группа-Номер)
...
protected $fillable = [
    ...
    "accounting_number",   // Учётный номер (система ICT Help)
    // ... другие поля
];
```
...
- **Учётный номер**: Зеленая панель с форматом системы ICT Help
...
- По учётному номеру (система ICT Help)
